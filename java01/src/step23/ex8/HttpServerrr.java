...
public class HttpServerrr {
	int port;
	
	public HttpServerrr(int port) {
		this.port = port;
	}
	...
	public static void main(String[] args) throws Exception {
		HttpServerrr server = new HttpServerrr(8888);
		server.listen();
	}
	...
	public void run () {
		...
		private String extractUrl(String requstLine) { 
			return requstLine.substring(requstLine.indexOf(" ") + 1, requstLine.lastIndexOf(" ")); // +1 해야지 그다음부터 마지막까지
		}	
		...
		private void ok(PrintStream out) {
			...
			out.println("</head>");
			out.println("<body>");
			out.println("		<h1>/ok를 요청하셨네요 ㅎㅎ</h1>");
			out.println("</body>");
			out.println("</html>");
			out.flush();
		}
