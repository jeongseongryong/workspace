/* service() 파라미터 II : ServletRequest III - 한들 파라미터 값 깨지는 현상
 ...
 *  -즉 GET 요청의 한글 값이 깨지는 경우는 프로그램 코드로 처리X 위와 같이  설정으로 처리
 * POST 요청의 파라미터 값 깨지는 경우
 * -프로그램 코드를 처리할 수 있다
 * -파라미터 값을 꺼내기전, 즉 getParamiter()를 최초로 호출하기 전에
 * 	클라이언트의 값이 어떤 문자집합으로 인코딩 되었는지 알려준 후에 꺼낸다.
 * 	예)req.setCharacterEncoding("UTF-8");
 * 
 * 	깨지는 이유
 *  "서블릿 컨테이너"는 클라이언트가 보낸는 값이 ISO-8859-1로 인코딩되어 있을 거라 가정한다.
 *  그 가정을 바탕으로 자바에서 사용할 수 있도록 "유니코드"로 변환
 *  예) 클라이언트가 "ABC가각간" 데이터를 보낼때,
 *  	41 42 43 EA B0 80 EA B0 81 EA B0 84
 *  서버에서 이데이터를 유니코드로 바꾸면(각 바이트에 00을 붙여 2바이트 유니코드로 만든다.)
...
public class Servlet03 extends GenericServlet {
	...
	public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
		...
		//req.setCharacterEncoding("UTF-8"); // getParameter() 먼저호출 하게되면 무조건 한글이 깨진다.
